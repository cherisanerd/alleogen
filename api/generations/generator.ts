// AEO File Generator — Deno CLI wrapper.
//
// This file is the single source of truth for generated-file content.
// It is invoked as a subprocess by api/generations/generate.php, which
// pipes a JSON payload on stdin and reads a JSON payload on stdout.
//
// Input (stdin, one JSON object):
//   {
//     "generation": {...},      // alleogen_generations row
//     "analysis":   {...|null}, // alleogen_analyses row (or null)
...
//     "zip_filename": "<safe filename>.zip"
//   }
//
// Run with Deno 1.30+. No npm install / node_modules required — Deno
// fetches jszip directly via npm: specifier and caches it.
//
// CLI: deno run --allow-net=deno.land,jsr.io generator.ts
// (Allowing net is needed once, while Deno fetches jszip to its cache.)
...
async function main() {
  ...
  try {
    ...
    emit({ success: false, error: 'Invalid JSON input: ' + (e as Error).message }, 2);
    return;
  }
  ...
  if (!generation || typeof generation !== 'object') {
    emit({ success: false, error: 'generation object is required' }, 2);
    return;
  }

  const website_url   = (generation.website_url   ?? '') as string;
  const business_name = (generation.business_name ?? '') as string;
  const package_tier  = (generation.package_tier  ?? 'basic') as string;

  // Analysis extracted_data may be JSON-encoded TEXT (MySQL JSON column returned
  // as a string) or already a parsed object.
  let analysisExtracted: Record<string, unknown> = {};
  const ed = (analysis as any)?.extracted_data;
  if (typeof ed === 'string') {
    try { analysisExtracted = JSON.parse(ed); } catch { analysisExtracted = {}; }
  } else if (ed && typeof ed === 'object') {
    analysisExtracted = ed as Record<string, unknown>;
  }
  ...
  try {
    const result = await generatePackage({
      ...
      package_tier,
      data,
      ex: analysisExtracted as any,
    });
    emit(result, 0);
  } catch (e) {
    emit({ success: false, error: 'Generator error: ' + (e as Error).message }, 3);
  }
}
...
function emit(payload: unknown, exitCode: number) {
...
// ------------------------------------------------------------
// Core pipeline. Everything below this line is the unchanged v2
// file-generation logic, refactored only to receive its inputs as a
// function argument instead of via Base44 SDK calls, and to return
// the zip bytes + files map instead of uploading.
// ------------------------------------------------------------
...
async function generatePackage(input: GenerateInput): Promise<GenerateOutput> {
  const { website_url, business_name, package_tier, data, ex } = input;
  ...
  // Generate ZIP as Uint8Array, return base64 so stdout (text) can carry it.
  const zipUint8Array = await zip.generateAsync({ type: 'uint8array' });
  const safeName = (business_name || 'package').replace(/[^a-z0-9]/gi, '_') || 'package';
  const zipFileName = `${safeName}_AEO_Package_${package_tier}.zip`;
  const zipBase64 = base64Encode(zipUint8Array);
...
// Small, dependency-free base64 encoder for Uint8Array → string.
function base64Encode(bytes: Uint8Array): string {
  ...
  // btoa is available in Deno's global namespace.
  return btoa(binary);
}

main().catch((e) => {
  emit({ success: false, error: 'Top-level error: ' + (e as Error).message }, 4);
});